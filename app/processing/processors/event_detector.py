...
class EventDetector(Processor):
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "event_keys": list(self._event_keys),
            "extrapolating_count": self._extrapolating_count,
            "was_extrapolating": self._was_extrapolating,
            "session_badge": self._session_badge,
            "is_sprint_quali": self._is_sprint_quali,
            "current_p1": self._current_p1,
            "tyres_published": self._tyres_published,
            "driver_compounds": dict(self._driver_compounds),
            "stopped_drivers": list(self._stopped_drivers),
            "first_wet_switch": self._first_wet_switch,
            "first_dry_switch": self._first_dry_switch,
            "last_rcm_key": self._last_rcm_key,
            "driver_tlas": dict(self._driver_tlas),
        }

    def restore(self, state: dict[str, Any]) -> None:
        self._events = state.get("events", [])
        self._event_keys = set(state.get("event_keys", []))
        self._extrapolating_count = state.get("extrapolating_count", 0)
        self._was_extrapolating = state.get("was_extrapolating", False)
        self._session_badge = state.get("session_badge")
        self._is_sprint_quali = state.get("is_sprint_quali", False)
        self._current_p1 = state.get("current_p1")
        self._tyres_published = state.get("tyres_published", False)
        self._driver_compounds = state.get("driver_compounds", {})
        self._stopped_drivers = set(state.get("stopped_drivers", []))
        self._first_wet_switch = state.get("first_wet_switch", False)
        self._first_dry_switch = state.get("first_dry_switch", False)
        self._last_rcm_key = state.get("last_rcm_key", -1)
        self._driver_tlas = state.get("driver_tlas", {})

    def reset(self) -> None:
        # Events persist (they're from the full scan), but tracking state resets
        self._extrapolating_count = 0
        self._was_extrapolating = False
        self._current_p1 = None
        self._tyres_published = False
        self._driver_compounds = {}
