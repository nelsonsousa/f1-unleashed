...
Emits:
  - trackGeometry   (corners and sectors as % of lap distance) — once
  - position        (all cars: x, y, distPct) — on each Position.z change

Loads the track SVG on SessionInfo to build the track polyline, then
projects each car's X,Y onto it to compute distance as % of lap.
Skips messages where no car has moved.
"""
