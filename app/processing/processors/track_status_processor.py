...
class TrackStatusProcessor(Processor):
    ...
    # snapshot/restore/reset are unused (DB latest-row-per-topic model);
    # kept only to satisfy the base ABC. Removed in the snapshot-removal branch.
    def snapshot(self) -> dict[str, Any]:
        return {
            "status": self._status,
            "message": self._message,
            "event_colour": self._event_colour,
        }

    def restore(self, state: dict[str, Any]) -> None:
        self._status = state.get("status", "")
        self._message = state.get("message", "")
        self._event_colour = state.get("event_colour")

    def reset(self) -> None:
        self._status = ""
        self._message = ""
        self._event_colour = None
