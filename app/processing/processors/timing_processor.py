...
class TimingProcessor(Processor):
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "lap_state": copy.deepcopy(self._lap_state),
            "laps": dict(self._laps),
            "stint_state": {k: {"raw": copy.deepcopy(v["raw"]), "emitted_stints": list(v["emitted_stints"])} for k, v in self._stint_state.items()},
            "best_laps": copy.deepcopy(self._best_laps),
            "lap_times": {k: dict(v) for k, v in self._lap_times.items()},
            "ll_count": dict(self._ll_count),
            "sticky_ll": dict(self._sticky_ll),
            "prev_nl": dict(self._prev_nl),
            "under_red_flag": self._under_red_flag,
            "standalone_since_nl": dict(self._standalone_since_nl),
        }

    def restore(self, state: dict[str, Any]) -> None:
        self._state = copy.deepcopy(state.get("state", {}))
        self._lap_state = copy.deepcopy(state.get("lap_state", {}))
        self._laps = state.get("laps", {})
        raw_stint = state.get("stint_state", {})
        self._stint_state = {k: {"raw": copy.deepcopy(v["raw"]), "emitted_stints": set(v.get("emitted_stints", []))} for k, v in raw_stint.items()}
        self._best_laps = copy.deepcopy(state.get("best_laps", {}))
        self._lap_times = {k: {int(lk): lv for lk, lv in v.items()} for k, v in state.get("lap_times", {}).items()}
        self._ll_count = dict(state.get("ll_count", {}))
        self._sticky_ll = dict(state.get("sticky_ll", {}))
        self._prev_nl = dict(state.get("prev_nl", {}))
        self._under_red_flag = bool(state.get("under_red_flag", False))
        self._standalone_since_nl = dict(state.get("standalone_since_nl", {}))

    def reset(self) -> None:
        self._state.clear()
        self._lap_state.clear()
        self._laps.clear()
        self._stint_state.clear()
        self._best_laps.clear()
        self._lap_times.clear()
        self._ll_count.clear()
        self._sticky_ll.clear()
        self._prev_nl.clear()
        self._under_red_flag = False
        self._standalone_since_nl.clear()
