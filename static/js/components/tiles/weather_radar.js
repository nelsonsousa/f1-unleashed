...
(function() {
    ...
    // (Mini-SVG loader removed — see extractMainTrackDimensions above.
    // The block immediately below is a no-op kept only so the diff to
    // the prior version stays small; safe to delete in a follow-up.)
    messageBus.on('state:never-fires-noop-marker', async () => {
        if (state.trackSvgLoaded) return;

        const params = new URLSearchParams(window.location.search);
        const sessionName = params.get('session') || '';
        // Extract location from any of:
        //   replay:    "2026/1286_Monte_Carlo/11292_Practice_1"
        //   home:      "2026_8_Monte_Carlo_Practice_1"
        //   cache:     "2026_1286_Monte_Carlo_11292_Practice_1"
        // All produce location='Monte_Carlo'. Kept in sync with track_map.js.
        let location = null;
        if (sessionName.includes('/')) {
            ...
            for (const suf of suffixes) {
                if (s.endsWith('_' + suf)) { s = s.slice(0, -suf.length - 1); break; }
            }
            const p = s.split('_');
            while (p.length >= 4 && /^\d+$/.test(p[p.length - 1])) p.pop();
        ...
        const filename = mappedName || location.normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, '_');
        ...
        try {
            ...
            // Strip car-markers and the wide grey outline — only the inner
            // red track line remains. At weather-tile scale the double
            // line is illegible; one suffices.
            const markers = svgEl.querySelector('#car-markers');
            if (markers) markers.remove();
            const outline = svgEl.querySelector('#track-outline');
            if (outline) outline.remove();

            // Tag this SVG so weather-tile-specific stroke rules pick it
            // up (vector-effect keeps the stroke a fixed pixel width).
            svgEl.classList.add('weather-track-svg');
            svgEl.style.width = '100%';
            svgEl.style.height = '100%';
            svgEl.style.opacity = '0.45';

            const container = document.getElementById('weatherRadarMap');
            if (container) {
                container.insertBefore(svgEl, container.firstChild);
                state.trackSvgLoaded = true;
                // Compute physical extent of the SVG so we can align it
                // with the radar tile at the same metres-per-pixel scale.
                // SVG raw coords are in 0.1 m units; the track-root group
                // carries data-scale = viewBox-units-per-raw-unit.
                const vb = (svgEl.getAttribute('viewBox') || '').split(/\s+/).map(Number);
                const dataScale = parseFloat(
                    svgEl.querySelector('#track-root')?.dataset?.scale || ''
                );
                if (vb.length === 4 && dataScale > 0) {
                    state.svgWidthM = vb[2] / dataScale / 10;
                    state.svgHeightM = vb[3] / dataScale / 10;
                    state.svgViewBoxAspect = vb[2] / vb[3];
                    maybeApplyAlignment();
